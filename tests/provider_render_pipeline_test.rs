...
fn usage_pipeline_single_provider_to_robot_markdown() {
    ...
    let md_output = robot::render_markdown_usage(&[payload])
...
fn usage_pipeline_multi_provider_aggregation() {
    ...
    log.phase("render_human");
    let human_output = human::render_usage(&providers, false)
        .expect("Multi-provider human render should succeed");
...
fn usage_pipeline_preserves_all_rate_windows() {
    ...
    assert!(usage["primary"].is_object(), "Primary window should be present");
    assert!(usage["secondary"].is_object(), "Secondary window should be present");
    assert!(usage["tertiary"].is_object(), "Tertiary window should be present");
    log.finish_ok();
}
...
fn cost_pipeline_single_provider_to_human_output() {
    ...
    log.phase("render_human");
    let human_output = human::render_cost(&[payload], false)
        .expect("Cost human render should succeed");
...
fn cost_pipeline_single_provider_to_robot_json() {
    ...
    let json_output = robot::render_cost_json(&[payload], false)
        .expect("Cost JSON render should succeed");
...
fn cost_pipeline_single_provider_to_robot_markdown() {
    ...
    log.phase("render_markdown");
    let md_output = robot::render_markdown_cost(&[payload])
        .expect("Cost markdown render should succeed");
...
fn cost_pipeline_multi_provider_aggregation() {
    ...
    log.phase("render_human");
    let human_output = human::render_cost(&providers, false)
        .expect("Multi-provider cost render should succeed");
...
fn usage_round_trip_serialization() {
    ...
    log.phase("serialize");
    let json = robot::render_usage_json(&[original.clone()], false)
        .expect("Serialization should succeed");

    log.phase("deserialize");
    let parsed: RobotOutput<Vec<ProviderPayload>> = serde_json::from_str(&json)
...
fn cost_round_trip_serialization() {
    ...
    log.phase("serialize");
    let json = robot::render_cost_json(&[original.clone()], false)
        .expect("Serialization should succeed");

    log.phase("deserialize");
    let parsed: RobotOutput<Vec<CostPayload>> = serde_json::from_str(&json)
        .expect("Deserialization should succeed");
...
fn same_data_renders_consistently() {
    ...
    let human_output = human::render_usage(&[payload.clone()], true)
        .expect("Human render should succeed");
    let json_output = robot::render_usage_json(&[payload.clone()], false)
